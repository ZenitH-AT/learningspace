...
if (isset($_GET['error'])) {
    ...
    if ($error == 1) {
        ?>
        <script>alert("Your credentials are incorrect. Please try signing in again with the correct information.");</script>
        <?php
    }
    if ($error == 2) {
        ?>
        <script>alert("Your email is incorrect. Please try again using the correct email.");</script>
        <?php
    }
    if ($error == 3) {
        ?>
        <script>alert("A reset password link was sent to your email.");</script>
        <?php
    }
    if ($error == 4) {
        ?>
    ...
    if ($error == 5) {
        ?>
        <script>alert("Your account has not yet been activated. Please check your email.");</script>
        <?php
    }
    if ($error == 6) {
        ?>
    ...
    if ($error == 7) {
        ?>
        <script>alert("Internet connection error or the provided email was invalid.");</script>
        <?php
    }
    if ($error == 8) {
        ?>
        <script>alert("Your password has been changed. Please sign in again using the new password.");</script>
        <?php
    }
}
?>
